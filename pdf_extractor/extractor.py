...
class PDFExtractor:
    ...
    def _find_nearest_value(self, label: dict) -> None:
        ...
        logger.info(
            f"[LABEL] {label['label_for']} | text='{label['text']}' | font='{label['font']}' | page={label['page']} | bbox={label['bbox']}"
        )
        ...
        for d, can in scored:
            logger.info(
                f"  -> candidate: '{can['text']}' | dist={d:.2f} | font='{can['font']}' | bbox={can['bbox']}"
            )
        nearest_dist, nearest = scored[0]
        logger.info(
            f"[MATCH] {label['label_for']} -> '{nearest['text']}' (dist={nearest_dist:.2f})"
        )
